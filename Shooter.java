...
public class Shooter { // velocity * 1.25 instead of addition
    ...
    // final static float y = 104.0f; // in // upper hub height by default. Should
    // have button to switch between // them. + 10.0f for ball
    final static float y = 104.0f;
    // final static float y = 41.0f;
    final static float y0 = 22.5f; // in
    final static float minimumVelocity = 267.7f;
    // final static float minimumVelocity = 50.0f;
    ...
    static float angleError = -0.87f; // was 0.04
    ...
    // public static float shooterWheelLinearVelocityToMotorVelocity(double
    // projectileVelocity) {
    // return (float) (1.9f * (projectileVelocity * (1.0f / (4.0f * Math.PI)) *
    // (1.0f / 1.21) * 2048.0f * (1.0f / 10.0f)));
    // // return (float) ((1.9f * (projectileVelocity * (1.0f / (4.0f * Math.PI)) *
    // (1.0f / 1.21) * 2048.0f * (1.0f / 10.0f))) / 18000.0f);
    ...
    public static float shootingCoefficient = 2.6f;
    ...
    public void shootInit() {
        ...
        calculatedDistance = getXDistanceFromCenterOfHub(
    ...
    public void testingShooter() {
        ...
        if (Constants.xbox.getRawButtonPressed(3)) { // Constants.xbox.getRawButtonPressed(3)
            if (!dumpShot) {
                shooting = !shooting;
                if (shooting) {
                    shootInit();
        ...
        if (shooting) {
            ...
            // if ((currentDistance < (calculatedDistance - 12.0f)) || (currentDistance >
            // (calculatedDistance + 12.0f))) {
            // shootInit();
            // }

            if (NetworkTableInstance.getDefault().getTable("limelight").getEntry("tx").getDouble(0.0) < -4.0) {
                ...
                isTargetCenter = false;
            } else if (NetworkTableInstance.getDefault().getTable("limelight").getEntry("tx").getDouble(0.0) > 4.0) {
                if (DriverStation.isTeleop()) {
                    Robot.drive.driveTrainByControls(Constants.stick.getRawAxis(1), Constants.stick.getRawAxis(0),
                            0.125,
                            true);
            ...
            if (!dumpShot) {
                masterShooterMotor.set(ControlMode.PercentOutput, 0.2);
                Robot.intake.indexerMotor.set(0.0);
            }
        }
    ...
    public double dumpShotSpeed = 0.4125; // hood all the way down

    public void dumpShot() { // do we even have this method enabled?
        ...
        if (dumpShot && !shooting) {
            ...
            if (DriverStation.isTeleop()) {
                Robot.drive.driveTrainByControls(Constants.stick.getRawAxis(1), Constants.stick.getRawAxis(0), 0.125,
                        true);
            } else {
                Robot.drive.driveTrainByControls(0.0, 0.0, 0.125, true);
            }
        ...
        // if ((hoodMotor.getEncoder().getPosition() < 10.5)
        // && (goDown == false)) {
        // goUp = true;
        // }
        ...
        // if ((hoodMotor.getEncoder().getPosition() > 10.8) && (goUp == false)) { // only goes down to it
        //     goDown = true;
        // }
        ...
        if ((masterShooterMotor.getSelectedSensorVelocity() > (dumpShotSpeed * 22201.15 * 1.006)) && setHoodYet
                && isTargetCenter) {
            Robot.intake.indexerMotor.set(-0.4);
        } else {
            Robot.intake.indexerMotor.set(0.0);
 ...
 * // }
 * 
 * if ((Robot.intake.ballCount > 0) && Constants.stick.getRawButtonPressed(2) &&
 * !shooting) {
 * shooting = true;
 ...
 * if (!NetworkTableInstance.getDefault().getTable("limelight").getEntry("tv").
 * getBoolean(false)) {
 * Robot.drive.mecanumDrive.driveCartesian(0.0, 0.0, 0.15); // arbitrary 0.3,
 * just says how fast to turn in
 ...
 * } else {
 * Robot.drive.stopMotors();
 * testVelocity((float) getXDistanceFromCenterOfHub((float)
 ...
 * // shooterWheelLinearVelocityToMotorVelocity(calculatedVelocity)); // should
 * be
 * // the speed according to some formula that sets it to the // initial
 * velocity
 * // we need
 * masterShooterMotor.set(ControlMode.PercentOutput,
 * (shooterWheelLinearVelocityToMotorVelocity(calculatedVelocity) / 18850.6));
 * if ((hoodMotor.getEncoder().getPosition() <
 * (Math.toDegrees(calculatedAngle))) && !setHoodYet) {
 * hoodMotor.set(0.1);
 * setHoodYet = false;
 * } else {
 * hoodMotor.set(0.0);
 ...
 * // calculatedAngle, hoodMotor.getEncoder().getPosition())); // what is the
 * best
 * // way to do this
 * if ((masterShooterMotor
 * .getSelectedSensorVelocity() >
 ...
 * // 1: use a timer and wait a certain amount of time (could be constant or
 * based
 * // on wanted motor speed)
 * // 2: use an if statement and verify the speed is what we want. Contact with
 * the
 ...
 * 
 * if ((hoodMotor.getEncoder().getPosition() < (90.0 -
 * Math.toDegrees(calculatedAngle))) && (goDown == false)) {
 * goUp = true;
 * hoodMotor.set(0.1);
 * if ((hoodMotor.getEncoder().getPosition() > (90.0 -
 * Math.toDegrees(calculatedAngle)))) {
 ...
 * 
 * if ((hoodMotor.getEncoder().getPosition() > (90.0 -
 * Math.toDegrees(calculatedAngle))) && (goUp == false)) {
 * goDown = true;
 * hoodMotor.set(-0.1);
 * if ((hoodMotor.getEncoder().getPosition() < (90.0 -
 * Math.toDegrees(calculatedAngle)))) {
 ...
 * if ((masterShooterMotor
 * .getSelectedSensorVelocity() >
 * shooterWheelLinearVelocityToMotorVelocity(calculatedVelocity) - 15.0)
 * && setHoodYet) {
 * Robot.intake.indexerMotor.set(-0.3);
 * } else {
 * Robot.intake.indexerMotor.set(0.0);
